...
const UseCaseRepository = require('../../contracts/UseCaseRepository');
const { SupabaseBaseRepository } = require('./SupabaseBaseRepository');

class SupabaseUseCaseRepository extends UseCaseRepository {
    constructor({ base } = {}) {
        super();
    ...
    async listByProject(projectId, exec) {
        const r = await this._query(`SELECT * FROM qa_use_cases WHERE project_id = ?`, [projectId]);
        return r.rows;
    }
    async listByProjectWithUSCount(projectId, exec) {
    ...
    async findById(id, exec) {
        const r = await this._query(`SELECT * FROM qa_use_cases WHERE id = ?`, [id]);
        return r.rows[0] || null;
    }
    async findByIdWithProject(id, exec) {
        const r = await this._query(
    ...
    async findProjectId(ucId, exec) {
        const r = await this._query(`SELECT project_id FROM qa_use_cases WHERE id = ?`, [ucId]);
        return r.rows[0]?.project_id;
    }
    async create({ projectId, keyId, title, description, createdBy, updatedBy }, exec) {
        const r = await this._query(
    ...
    async update(id, { title, description, status, keyId, updatedBy }, exec) {
        await this._query(
            `UPDATE qa_use_cases
    ...
    async remove(id, exec) {
        await this._query(`DELETE FROM qa_use_cases WHERE id = ?`, [id]);
    }
    async coverageByProject(projectId, exec) {
        const r = await this._query(
...
module.exports = { SupabaseUseCaseRepository };
